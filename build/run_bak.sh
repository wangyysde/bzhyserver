...
function build::action::head()
{
  /usr/bin/cat >"${SYSADM_ROOT}/Makefile" <<EOF
###
# SYSADM Server
...
LDFlags="-X 'config.Commit=\${GITCOMMIT}' -X 'config.BuildBranch=\${BranchInfo}' -X 'config.Buildstamp=\${BUILDTIME}' -X 'config.goversion=\${GOVERSION}' "
...
function build::action::all(){
  /usr/bin/cat >>"${SYSADM_ROOT}/Makefile" <<EOF
all: server
EOF
}

function build::action::clean(){
  /usr/bin/cat >>"${SYSADM_ROOT}/Makefile" <<EOF
